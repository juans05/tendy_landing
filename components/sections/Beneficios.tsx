  ...
  return (
    <section id="beneficios" className="bg-gray-50 px-4 py-16">
      <div className="mx-auto max-w-5xl">
        <h2 className="mb-8 text-center text-2xl font-bold text-brand-black sm:text-3xl">{content.beneficios.titulo}</h2>
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {content.beneficios.items.map((item, index) => (
            ...
            >
              {item.icon === 'trophy' && (
                <span className="absolute -right-2 -top-2 rounded-full bg-brand-orange px-2 py-0.5 text-xs font-bold text-brand-black shadow">
                  🎉
                </span>
              )}
              <Icon name={item.icon} className={`mb-3 h-8 w-8 ${ICON_COLORS[index % ICON_COLORS.length]}`} />
              <h3 className="mb-2 text-lg font-bold text-brand-black">{item.titulo}</h3>
